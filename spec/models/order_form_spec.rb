  ...
  describe '配送先情報の保存' do
    ...
    context '配送先情報の保存ができないとき' do
      ...
      it '郵便番号が空だと保存できないこと' do
        ...
        @order_form.valid?
        expect(@order_form.message.errors).to include("Postcode can't be blank")
      end
      it '郵便番号にハイフンがないと保存できないこと' do
        @order_form.postcode = 1_234_567
        @order_form.valid?
        expect(@order_form.message.errors).to include('Postcode is invalid. Include hyphen(-)')
      end
      it '都道府県が「---」だと保存できないこと' do
        @order_form.prefecture_id = 0
        @order_form.valid?
        expect(@order_form.message.errors).to include("Prefecture can't be blank")
      end
